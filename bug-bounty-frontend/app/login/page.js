  ...
  return (
    <Bug
      ...
      style={{
        ...
        filter: `drop-shadow(0 0 8px ${Math.random() > 0.5 ? "#ff0080" : "#8000ff"})`,
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        window.sessionStorage?.setItem("token", data.token) || (window.userToken = data.token);
  ...
  return (
    <div 
      ...
      style={{
        background: "radial-gradient(circle at 20% 50%, #1a0033 0%, #000000 25%, #0d0d0d 50%, #000000 100%)",
    ...
    >
      <style jsx>{`
        ...
        .input-glow:focus {
          box-shadow: 0 0 20px rgba(128, 0, 255, 0.3), 0 0 40px rgba(255, 0, 128, 0.2);
        }
      `}</style>
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        {/* Login Card */}
        <Card className="bg-black/90 shadow-2xl border-2 border-purple-500/20 rounded-3xl overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:border-purple-400/30" style={{ backdropFilter: 'blur(20px)' }}>
          ...
          <CardFooter className="flex justify-center pt-4 pb-8">
            ...
            >
              New Hunter? <span className="text-pink-400 font-bold ml-2">Register →</span>
            </Button>
          </CardFooter>
        </Card>
